...
export function BillingClient({ wallet, currentTopup, recentTopups }: { wallet: { available_micros: string; reserved_micros: string } | null; currentTopup: TopupData | null; recentTopups: TopupData[] }) {
  ...
  const packages = [
    { amount: 20000, credits: '20 Credit', bonus: 'Bắt đầu' },
      ...
      <div className="billing-grid">
        ...
        <section className="surface surface-pad">
          ...
          <div className="amount-grid">
            {packages.map((pkg) => <button key={pkg.amount} type="button" className={`amount-choice ${selectedAmount === pkg.amount ? 'active' : ''}`} onClick={() => setSelectedAmount(pkg.amount)}><strong>{formatVnd(pkg.amount)}</strong><small>{pkg.credits}</small>{pkg.bonus && <small className="bonus">{pkg.bonus}</small>}</button>)}
          </div>
